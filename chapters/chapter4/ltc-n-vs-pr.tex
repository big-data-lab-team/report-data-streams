...
\TG{Title syntax should match the one of 4.1}
...
curves \TG{Do you mean polynomials? I'm not sure what's a plane curve}.
...
variables $a.x$, $a.y$ or $a.z$ (e.g. $t \sim a.x$) \TG{what do you mean ``with the relationship''?}.
...
regression models of parameters $x$, $y$ and $z$ are calculated \TG{Are they recalcultated each time a new point arrives?}. The function
...
where should the model stop \TG{What do you mean ``stop''?}. In the Table \TG{which one?}, it is obvious that LTC n-dimension work
better \TG{what does ``work better'' mean?} than Polynomial regression, because it needs more bytes to save the
coefficients. In our case, coefficients are recorded as float points \TG{A ``float point'' doesn't exist. Do you mean ``single-precision floating point numbers''?} which would
use 4 bytes per each. The total number of bytes needed to represent the original
data is (N+1)*M*4+4 (Int32 for Time-stamp \TG{I don't understand this parenthesis, explain}). Polynomial regression uses more
processing time \TG{more than what?} to calculate coefficients and build model when each new data
point comes. 
...
different dimensions. \TG{This is very surprising, as a straight line is a particular case of polynomial of degree 3 or 5.} Besides,
...
variations are slight. \english{However, in Table~\ref{table:CR-compare-walking}, when
dimension is 2 and 3, the 5-degree regression compresses more data
...
for infinity norm.} \TG{Split this long sentence in smaller, simpler ones.}
