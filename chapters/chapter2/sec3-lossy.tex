...
In this thesis, we only focus on the lossy compression method, because \english{the
measure from sensor devices and the accurate value are not always same} \TG{I don't understand what you mean}, and
...
order to save more energy, we prefer a compression method with low computational
complexity \english{so that decreasing the energy cost of compression process}. LTC takes
...
\frac{hp_{3}+lp_{3}}{2})$. \TG{There's no high line or low line in Figure 3. The labels on Figure 3 don't
...
zordan2014performance} \TG{This sentence has no verb}. 
...
model respectively, $Z_{t-q}, ...,Z_{t}$ are white noise (usually \english{understand} as
