...
% go forward (read subsequent data into look-ahead buffer) \TG{\#\#what do you mean by ``move forward?''}, function \texttt{go\_forward($D$, $S$, $m$)} is invoked,
...
Table~\ref{table:LEC} is used for compression process in~\cite{marcelloni2008simple, marcelloni2009efficient}. It
...
The Sequential Lossless Entropy Compression (S-LEC) algorithm is an extension and improvement of LEC Algorithm by Li et
