...
The LEC algorithm was first proposed by Marcelloni et
...
size is determined by the number of bits after ADC \TG{expand acronyms}
...
In~\cite{marcelloni2008simple}, the authors generated the $s_i$ from
$n_i$ by Huffman coding. \english{Let's say the $s_i$ is the representation at $n_i$ in
...
process, \english{because of the correlation characteristic of sensor stream, e.g. the
different of stream data unlikely be too large}. In LEC, the result after
encoding has two parts, $s_i$ and $ a_i$, and the $s_i$ part causes \TG{do you mean contains?}
...
(Square Mean Error)~\cite{li2016temporal} \TG{what does ``be in the
same SME'' mean?}. We need to save the table into memory when we uses these
