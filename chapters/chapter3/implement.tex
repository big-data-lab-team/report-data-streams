...
11, and a maximum length $L_s$ of the set of intersecting n-balls $S$ is
assigned in order to restrict memory usage. Secondly, for each $\mathcal{B}_i$
in $S$, we check the intersection between $\mathcal{B}_i$ and $\mathcal{B}_j$
(line 14 in Algorithm~\ref{algo:euclidean}). At the same time, $\mathcal{B}_i$
will be removed if it includes $\mathcal{B}_j$, because once $\mathcal{B}_j$ has
an intersection with n-balls in $S$, then a bigger n-ball which contains the
$\mathcal{B}_j$ must have an intersection with them too.
...
in $S$ has an intersection. Therefore, any point in the intersection of
