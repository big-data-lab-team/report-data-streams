...
B)}. \TG{This paragraph has overlap with the previous one, writing should be revised
such that the description of the algorithm is straight and unambiguous.} Our code is available at
\url{https://github.com/big-data-lab-team/stream-summarization} under MIT
license. \TG{You should mention that either at the beginning or at the end of the
section, not right in the middle.}

\TG{Use ``n-ball'' instead of ``ball'' everywhere.}
...
Also, add a reference to plane sweep.}
...
\If{$\exists\ \mathcal{B}_i \in S$ s.t. $\mathcal{B}_j \cap \mathcal{B}_i = \O$}
    \State \Return ($S$, $B$, False) \Comment{Some n-balls don't intersect}
\EndIf
...
\begin{algorithmic}[1]
\Input
   \Desc{$S$}{Set of intersecting n-balls} \TG{I don't think you know that they are intersecting at this point.
   It should be candidate n-balls.}
   \Desc{$B$}{Bounding box of the intersection of n-balls in $S$}
\EndInput
\Output
   \Desc{$X$}{Point in all n-balls in $S$, $X = \{x_1,...,x_n\}$}
\EndOutput
...
\begin{algorithmic}[1]
\Input
   \Desc{$S$}{Set of intersecting n-balls} \TG{There's no guarantee that they intersect when this function is called.}
   ...
   \Desc{$X$}{Point in all n-balls in $S$, $X = \{x_1,...,x_n\}$} \TG{If you know the point already, what are you looking for?}
\EndInput
\Output
   \Desc{$T$}{True if a point is found by using bisection}
\EndOutput
\If{$N$ == 1}
    \State $X.x_1$ = $\frac{L+R}{2}$ \TG{isn't it X.x_N?}
    \State \Return True
\EndIf
\While{$L \leqslant R$ }
