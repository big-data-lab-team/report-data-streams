...
maintaining boundary lines, as detailed previously~\ref{sec:ltc} \TG{\#\#Refer to specific chapter and section}. In higher 
...
According to equations~\eqref{eq:ltc-2} and~\eqref{eq:ltc-3} in section~\ref{sec:ltc}, let $(u_0^k, y_0^k) \in \mathbb{R}^{n+1}$ be the latest transmitted point. For convenience, all the subsequent points will be
expressed in the orthogonal space with origin $(\tau_k, \xi_k)$ through equation~\eqref{eq:ltc-3}. We denote by $(v_j, z_j)_{j \in \llbracket 0, m \rrbracket}$ such points:
\begin{equation}
...
Euclidean norm (see Figure~\ref{fig:norm-comparison-2D}). In 3D, this ratio would be $\frac{6}{\pi}$ (see Figure~\ref{fig:norm-comparison-3D}). Conversely, 
...
the Euclidean norm. \TG{??This could be clarified, e.g., through a figure.}\todo{cannot understand this sentence.} Unsurprisingly, the
...
\centering
    \begin{subfigure}{.5\textwidth}
        \centering
        \includegraphics[width=0.5\textwidth]{figures/2D_Comparison.png}
        \caption{Comparison of compression ratio in 2D}
    ...
    \begin{subfigure}{.5\textwidth}
        \centering
        \includegraphics[width=0.5\textwidth]{figures/2D_Comparison.png}
        \caption{Comparison of compression ratio in 3D}
        \label{fig:norm-comparison-3D}
    \end{subfigure}

\caption{Comparisons between Infinity norm and Euclidean norm}
